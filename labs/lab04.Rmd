...
b_0 is the intercept of our function that will predict the height of the child. b_1 is the change in height for the child. The RMSE the variation of the childs height. R^2 tells us that the proportion of variance explained is only about 21%.

How good is this model? How well does it predict? Discuss.

Our model is a good model. The error in the predicted height is only about  +/-2.24 inches which is not a significantly large error. 
...
It is reasonable that parents and their children have the same height because childern inherit parents DNA which play an important role on the childs height.

If they were to have the same height and any differences were just random noise with expectation 0, what would the values of $\beta_0$ and $\beta_1$ be?

The values of both beta_0 and beta_1 would be 0 because knowing the parents height we know the childs height.
...
It was shortened to "regression to the mean" beause Galton observed that after an extreme result in the height of the first child then the height of next the child will then be closer to the mean, thus regressing to the mean.

Why should this effect be real?

This effect should be real because through Galton's findings, it shows how the heights of the childern of the same parents got closer to the mean after the first. 

You now have unlocked the mystery. Why is it that when modeling with $y$ continuous, everyone calls it "regression"? Write a better, more descriptive and appropriate name for building predictive models with $y$ continuous.

When modeling with y continuous it is called regression because the more sampling that is done the closer the results will be to the mean.
A better name for this is Linear Regression.
...
The difference between both matrcies is that in the second matrix we have a column for the setosa species, whereas in the first one we don't. 
