...
ts_hu_diameter%>%
  ...
  mutate(observation_period = 1:n(),ts_diameter) 
...
```{r}
ts_hu_diameter_long = 
  pivot_longer(ts_hu_diameter,cols=c(ts_diameter,hu_diameter), names_to = "diameter_type",values_to = "diameter")
...
```{r}
ggplot(ts_hu_diameter_long)+ 
  aes(x=observation_period)+
  geom_line(aes(y=ts_diameter), col = "red")+
  geom_line(aes(y=hu_diameter), col = "green")+
  ylab("diameter")
```